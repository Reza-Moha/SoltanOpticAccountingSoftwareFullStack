...
import EditeRefractiveIndexModal from "./EditeRefractiveIndexModal";

export function RefractiveIndexList() {
  const dispatch = useDispatch();
  const [showEditModal, setShowEditModal] = useState(false);
  const [selectedRefractiveIndex, setSelectedRefractiveIndex] = useState(null);
  ...
  const handleEditRefractiveIndex = (refractiveIndex) => {
    setSelectedRefractiveIndex(refractiveIndex);
  ...
  return (
    <div className="border-t border-secondary-500">
                ...
                <motion.tr key={refractiveIndex.id}>
                  ...
                  <td className="font-bold text-sm">
                    {refractiveIndex.characteristics.length > 0
                      ? refractiveIndex.characteristics.map((item) => {
                          return (
                            <h3 className="text-xs mb-1.5 border-r-2 border-indigo-400 pr-1">
                  ...
                  <td className="flex items-center gap-x-4">
                    ...
                    <button
                      ...
                      onClick={() => handleEditRefractiveIndex(refractiveIndex)}
                    >
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        viewBox="0 0 24 24"
      ...
      {selectedRefractiveIndex && showEditModal && (
        <EditeRefractiveIndexModal
          RefractiveIndex={selectedRefractiveIndex}
          show={showEditModal}
